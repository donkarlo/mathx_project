...
class RectangleTurnLeftParamShapePointGenerator(ParamShapePointGeneratorComposit):
    def __init__(self, distanceInterval):
        ...
        l7pg = ParamLinePointGenerator(l7, distanceInterval, 0, 3.33)

        l8 = ParamShapeLine([0, 0, 1], [6.66, 10, 10])
        l8pg = ParamLinePointGenerator(l8, distanceInterval, 0, 3.33)

        l9 = ParamShapeLine([-1, 0, 0], [6.66, 10, 13.33])
        l9pg = ParamLinePointGenerator(l9, distanceInterval, 0, 3.33)

        l10 = ParamShapeLine([0, 0, -1], [3.33, 10, 13.33])
        l10pg = ParamLinePointGenerator(l10, distanceInterval, 0, 3.33)

        l11 = ParamShapeLine([-1, 0, 0], [3.33, 10, 10])
        l11pg = ParamLinePointGenerator(l11, distanceInterval, 0, 3.33)
        #####
